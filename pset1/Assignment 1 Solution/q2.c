...
sorting_methods sorters[N_SORTERS] = {
    {"bubble", bubble_sort},
    {"bogo", bogo_sort},{"merge",merge_sort}
    };
...
int main(int argc, char ** argv) {
    ...
    for (int requested_sorter = 1; requested_sorter < argc; requested_sorter++ ) {
      ...
      if (contor==N_SORTERS)//use the predefined benchmark
      {
            for (int requested_sorter = 1; requested_sorter < argc; requested_sorter++ ) {
                  for (int sorter_idx = 0; sorter_idx < N_SORTERS; sorter_idx++) {
                        if (strcmp(argv[requested_sorter],sorters[sorter_idx].name) == 0)
            ...
            //otherwise use the code for the argv words, and all available sorters
                  ...
                  int new_elements=argc-1;//instead of N_ELEMENTS, use new_elements, the number of input words
                  for (int sorter_idx = 0; sorter_idx < N_SORTERS; sorter_idx++)
                  {
                        char ** copied_array = get_copied_array(new_elements,argv);//I replaced here data by argv
                        clock_t start_time = clock();
                        sorters[sorter_idx].fnc(new_elements, copied_array);
                        clock_t end_time = clock();
                        double time = ((double)(end_time - start_time))/CLOCKS_PER_SEC*1000;
                        bool correct = sorted_correctly(new_elements, copied_array);
                        print_line(sorters[sorter_idx].name,time,correct);
                        free_copied_array(new_elements,copied_array);
                  }
            }
